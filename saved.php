...
$sql = "SELECT plan.strtDate, plan.endDate, plan.noOfpeople, plan.htlBill, plan.foodBill, plan.totalBill, uplace.placeName FROM plan,uplace WHERE plan.endDate=uplace.endDate and plan.strtDate=uplace.strtDate and plan.pay='0' and plan.personId='$id' and uplace.personId='$id'";
...
if ("Cox's Bazar" == $dname) {
    ...
    $sql1 = "SELECT hotelcox.htlName, plan.htlId FROM hotelcox,plan WHERE plan.endDate='$end' and plan.strtDate='$std' and plan.pay='0' and plan.personId='$id' and plan.htlId=hotelcox.htlId";
...
if ("Sylhet" == $dname) {
    ...
    $sql1 = "SELECT hotelsylhet.htlName, plan.htlId FROM hotelsylhet,plan WHERE plan.endDate='$end' and plan.strtDate='$std' and plan.pay='0' and plan.personId='$id' and plan.htlId=hotelsylhet.htlId";
    $result1 = mysqli_query($con, $sql1);
    if (mysqli_num_rows($result1) > 0) {
        while ($row = mysqli_fetch_array($result1)) {
            $hname = $row['htlName'];
...
$sql1 = "SELECT foodcox.foodName FROM foodcox,plan WHERE plan.endDate='$end'and plan.strtDate='$std' and plan.pay='0' and plan.personId='$id' and plan.foodId=foodcox.foodId";
$result1 = mysqli_query($con, $sql1);
if (mysqli_num_rows($result1) > 0) {
    while ($row = mysqli_fetch_array($result1)) {
...
$sql1 = "SELECT foodcox.foodName FROM foodcox,plan WHERE plan.endDate='$end'and plan.strtDate='$std' and plan.pay='0' and plan.personId='$id' and plan.lunFoodId=foodcox.foodId";
$result1 = mysqli_query($con, $sql1);
if (mysqli_num_rows($result1) > 0) {
    while ($row = mysqli_fetch_array($result1)) {
...
$sql1 = "SELECT foodcox.foodName FROM foodcox,plan WHERE plan.endDate='$end'and plan.strtDate='$std' and plan.pay='0' and plan.personId='$id' and plan.dinFoodId=foodcox.foodId";
$result1 = mysqli_query($con, $sql1);
if (mysqli_num_rows($result1) > 0) {
    while ($row = mysqli_fetch_array($result1)) {
...
<div class="place">
		<br>
		<h1 class="h1" style="color: pink">Your Recent Plan</h1>
    ...
    echo "<br>";}?></td>
		...
		<h3 style="color: pink"> **Complete payment to confirm your plan</h3> <br>




<form action="payConfirm.php">
        <button type="submit" name="payment" class="pay">Payment</button>
        </form>
<br>
